...
echo "docker run --rm -d --network=jepsen --ip='$SEED' --name='tendermint0' -v './nodedirs/mach0:/root/.tendermint' tendermint/merkleeyes:latest 'jepsen0'"
docker run --rm -d --network=jepsen --ip="$SEED" --name="tendermint0" -v "./nodedirs/mach0:/root/.tendermint" tendermint/merkleeyes:latest "jepsen0" 
